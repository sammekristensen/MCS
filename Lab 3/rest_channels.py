        ...
        for person in range(0,self.population):
            ...
            while not placed:
                ...
                if location not in self.rest_indices:
                    ...
                    location = rest_index
                    self.rest_channel[location] = Person("infected") if random.random() < prop_infected else Person("susceptible")
                    rest_index += 1
                    placed = True
...
class Hex_grid:
    def __init__(self, right:int, bottom:int, r_val, a, rest_channels) -> None:    
        self.grid = {}
    ...
    def populate_grid(self, prop_infected=0.005):
        for cell in self.grid.values():
            cell.populate(prop_infected)

def run_simulation(steps, r, a, rest_channels):
    grid = Hex_grid(100,100,r,a, rest_channels)
    grid.populate_grid()
...
def plot_fig(steps,susceptible,infected,recovered):
...
if __name__ == "__main__":
    ...
    susceptible, infected, recovered = run_simulation(steps,r,a,rest_channels)

    plot_fig(steps,susceptible,infected,recovered)